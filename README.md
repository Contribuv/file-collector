...
<p align="center">
  <img src="https://img.shields.io/badge/version-2.3.27-blue" alt="version">
...
- 改用 `fcntl.flock` 文件锁确保多 worker 下只一个执行反代自动恢复

### v2.3.26
- 新增反代自动恢复：NAS 重启后自动用上次保存的配置重启反代，无需手动启动
- `post_worker_init` 中第一个 worker 延迟 3s 调用 `auto_recover()`，避免多 worker 重复启动
- `_try_recover_from_state` 优化：进程不存在时不再清除状态文件，保留 config 供自动恢复使用
- 新增 `auto_recover()` 方法：检查状态文件中是否有上次运行配置，Go 进程已死则自动重启
- 启动前检查证书文件是否仍存在，证书缺失则跳过恢复并清除状态
...
- **保留多媒体预览**：PDF（PDF.js 官方渲染）、图片、视频、音频、TXT（小说阅读器，章节识别）
- **TXT 小说阅读器**：暗色/亮色主题切换、字体大小/行宽/行高调节、章节自动检测与侧栏导航、GBK 编码自动识别
- **后台批量操作重构**：`/admin/records` 和 `/admin/links` 采用 `_isBatchMode` / `_selectedIds` Set 模式，默认隐藏复选框
- **批量操作底栏**：固定底栏显示选中数量，打包下载（含等待浮窗）、批量删除（Fetch+JSON）
- **分页缓存修复**：`set_user_setting` 写入后清除缓存，分页链接携带 `per_page` 参数，翻页保持每页条数
- **CSRF 安全增强**：批量删除/批量下载/详情浮窗统一 CSRF 校验，杜绝跨站请求伪造
- **移动端适配**：批量操作栏纵向堆叠、按钮全宽，复选框无批量模式时完全隐藏
...
- 反向代理配置页面全新设计：单栏分区布局、操作条同行显示、高级选项 auto-fit 横排
- 反向代理增加外网访问链接（新窗口打开）、日志实时刷新、自动滚动、清除/导出日志
- Gzip 压缩和 HSTS 默认启用，代理超时默认 600 秒，修复超时不重连提示
- 修复 Gunicorn 多 worker 下反代状态丢失：状态持久化到 DB + 端口检测双重保障
- 反代日志改为文件共享存储（DATA_DIR/reverse_proxy.log），跨 worker 实时可见
- 反代日志同步输出到 Python logger，与 app.py 保持一致
- 修复反向代理页 JS 语法错误、escapeHtml 转义缺失、Jinja2 循环变量错误
- 修复 HSTS 功能未实际生效、stop() 时序竞争条件
- 修复卸载时选择「删除数据库」后 DATA_DIR/backups 仍有备份残留的问题
...
- 修复「显示预览按钮」开关错误控制分享页的问题 — 该开关仅控制收集页，分享页始终显示预览和下载按钮
- 系统设置页「分页设置」卡片合并到「默认设置」卡片中，减少页面碎片化

### v2.3.5
- 增强 HEIC 图片转换路径遍历安全防护
- 全面规范化日志系统，所有日志统一通过 logger 输出
- 清理项目备份文件和无用测试残留，减小安装包体积
- 统一 pillow-heif 离线包版本依赖

### v2.3.1
- 修复收集页/分享页音频文件无法预览问题（后端返回 `as_attachment=False`）
- 修复分享页音频文件不显示预览按钮问题（前端 `canPreview` 判断漏了音频扩展名）
- 修复 Office 文件（PPT/PPTX）预览时高度未填满容器问题
- 修复 JitViewer 预览 PPT 时产生 `/view/undefined` 404 报错问题
- 统一所有页面 Office 文件超过 30MB 的提示为「文件过大，无法预览」
- 恢复 Git 原始版本的文件预览方案（JitViewer 新窗口预览）
- 优化上传进度条动画和速度计算算法
- 添加上传暂停/继续功能
- 上传中时禁止继续添加文件，优化上传区域交互
